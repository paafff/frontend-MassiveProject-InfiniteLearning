  ...
  return (
    <div className="w-full md:w-5/6 md:mx-auto my-24">
      <div className="w-full flex flex-col">
        <p className="text-center text-lg md:text-xl xl:text-2xl font-bold">
          Trend Rambut
        </p>
...
const Hair = ({ id, title, image, index }) => {
  return (
    <div
      className="min-h-[150px] min-w-[150px] xl:min-h-[200px] xl:min-w-[200px] p-2 flex justify-center items-end bg-[length:160px_160px] xl:bg-[length:200px_200px] bg-gray-500 rounded"
      style={{ backgroundImage: `url(${image})` }}
    >
      <Link
        to={`/hair-trend?index=${index}`}
        className="w-full text-center font-semibold text-xs lg:text-sm bg-white py-1 px-2 rounded"
      >
        {title}
      </Link>
    </div>
