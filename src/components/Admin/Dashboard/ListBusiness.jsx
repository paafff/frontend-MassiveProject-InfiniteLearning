...
const ListBusiness = ({ showSidebar, setShowSidebar, listBusiness, hamburgerMenu }) => {
    return (
        <div className='w-full lg:w-full xl:w-3/4 xl:px-24 py-10 px-5 md:px-12 lg:pt-16'>
            ...
            <div className='bg-white w-full rounded-lg p-4 lg:p-8 drop-shadow-md'>
                ...
                {listBusiness.map((business, index) => (
                    ...
                    <Business index={index} id={business.id} name={business.name} type={business.typeBusiness} date={business.created_at} />
...
const Business = ({ id, name, type, date, index }) => {
    return (
        <div className='w-full py-3 px-2 border-t border-gray-200 flex justify-start items-center gap-5' key={id}>
            ...
            <div className='w-full flex justify-between items-center'>
                <div className='w-1/4 md:w-1/3 flex flex-col gap-1 md:gap-2'>
                    ...
                    {type == "Barber" ? (
                        <p className='text-[7px] md:text-sm w-fit px-[5px] py-[3px]  rounded-sm bg-zinc-900 text-white'>{type}</p>
                    ) : (
                        <p className='text-[7px] md:text-sm w-fit px-[5px] py-[3px]  rounded-sm bg-rose-400 text-white'>{type}</p>
                    )}
                ...
                <div>
                    <p className='text-xs text-gray-400'>terdaftar <br /> {date}</p>
                </div>
            </div>
        </div>
    )
